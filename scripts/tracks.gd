...
func init(track_count: int, track_spacing: int, track_scene: PackedScene, spawn_mode: int, spawn_initial_interval_min: int, spawn_interval_min: int, spawn_interval_max: int, train_speed: float):
	for n in track_count:
		var track = track_scene.instantiate()
		self.add_child(track)
		track.set_spawn_mode(spawn_mode, n)
		track.set_spawn_interval(spawn_initial_interval_min, spawn_interval_min, spawn_interval_max)
